...
(function () {
  ...
  function shiftToPreset(shift) {
    if (!shift) return '';
    if (shift.status === '休み') return '休み';
    if (shift.status === '有給') return '有給';
  ...
  function ensureBulkUi() {
    ...
    if (!panel) return;
    let bulk = document.getElementById('nursery-bulk-section');
    if (bulk) return;

    bulk = document.createElement('section');
    ...
    bulk.innerHTML = `
      ...
      <div id="nursery-bulk-body" hidden>
        <div class="business-muted-note" style="margin-bottom:12px">
          まず曜日ごとの基本シフトを選び「曜日設定を反映」。そのあと例外の日だけ変更して、一度に保存できます。
        </div>
        ...
        <div style="display:flex;gap:8px;align-items:center;margin-bottom:14px;flex-wrap:wrap">
          ...
          <button class="btn btn-primary" id="nursery-save-month">選んだ日を一括保存</button>
  ...
  async function renderBulkPanel() {
    ...
    const ym = monthValue();
    const weekTemplate = document.getElementById('nursery-week-template');
    weekTemplate.innerHTML = WEEKDAYS.map((wd, index) => `
      <label style="display:block;font-size:12px;color:#94a3b8">
        ${wd}曜
    ...
    const rows = monthDates(ym).map(date => {
      ...
      return `
        <tr>
          ...
          <td>
            <select data-bulk-date="${date}" data-weekday="${parseIsoDate(date).getDay()}">${presetOptions(selected)}</select>
          </td>
          <td style="font-size:11px;color:#64748b">${saved && !selected ? '詳細設定あり（編集画面を優先）' : ''}</td>
        </tr>
      `;
    }).join('');
    ...
    document.getElementById('nursery-bulk-grid').innerHTML = `
      <table class="works-table" style="min-width:520px">
        <thead><tr><th>日付</th><th>基本シフト</th><th></th></tr></thead>
        <tbody>${rows}</tbody>
      </table>
    `;
  ...
  function applyWeekTemplate() {
    ...
    document.querySelectorAll('[data-week-template]').forEach(select => {
      template.set(Number(select.dataset.weekTemplate), select.value);
    });
    document.querySelectorAll('[data-bulk-date]').forEach(select => {
      const weekdayIndex = Number(select.dataset.weekday);
      if (template.has(weekdayIndex)) select.value = template.get(weekdayIndex);
    });
    document.getElementById('nursery-bulk-status').textContent = '曜日設定を反映しました。例外の日だけ変更してください。';
  }
  ...
  async function saveBulkMonth() {
    ...
    const rows = [...document.querySelectorAll('[data-bulk-date]')]
      .map(select => bodyFromPreset(select.dataset.bulkDate, select.value))
      .filter(Boolean);

    if (!rows.length) {
      status.textContent = '保存する日が選ばれていません。';
      return;
    }
    ...
    if (typeof lastNurseryMonth !== 'undefined') lastNurseryMonth = null;
    // business-custom.js 内の一覧は API 更新後のDOM変化で再読込されるため、直接リロードして確実に反映。
    try {
      const response = await fetch(`/api/nursery-shifts?month=${encodeURIComponent(monthValue())}`);
      const data = await response.json();
      if (response.ok && data.ok) {
        // 一覧側の内部変数には触れられないので、月ナビの軽い更新を利用する。
        const currentMonthEl = document.getElementById('current-month');
        if (currentMonthEl) currentMonthEl.textContent = currentMonthEl.textContent;
      }
    } catch (_) {}
  ...
  function init() {
    ...
    const container = document.getElementById('nursery-shift-container');
    if (container) {
      new MutationObserver(formatNurseryDates).observe(container, { childList: true, subtree: true });
    }

    ['prev-month', 'next-month'].forEach(id => {
      document.getElementById(id)?.addEventListener('click', () => setTimeout(renderBulkPanel, 80));
    });
  }
